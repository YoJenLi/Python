...
def display_top_features(weights, names, top_n, select=abs):
    ...
    ax.set_xticklabels(top_names, rotation=45, fontsize=12, fontdict={'fontsize':7})
...
display_top_features(svc.coef_[0], dvec.get_feature_names(), 60)


# In[18]:

# top positive features for posts
display_top_features(svc.coef_[0], dvec.get_feature_names(), 60, select=lambda x: x)


# In[19]:

# top features for comments
display_top_features(c_svc.coef_[0], c_dvec.get_feature_names(), 60)


# In[20]:

# top positive features for comments
display_top_features(c_svc.coef_[0], c_dvec.get_feature_names(), 60, select=lambda x: x)
